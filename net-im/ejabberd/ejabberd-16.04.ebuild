...
IUSE="captcha debug full-xml hipe ldap mssql mysql nls odbc pam postgres redis
	riak roster-gw sqlite test tools zlib"
...
src_prepare() {
	...
	# Suppress deps check.
	mkdir deps && :>"${S}/deps/.got" && :>"${S}/deps/.built"
	sed -e '10,53d' \
		-e '9a{deps, []}.' \
		-i "${S}/rebar.config"
	...
	# Fix up the ssl cert paths in ejabberd.yml to use our certificate.
	# Use the correct pam service name.
...
src_install() {
	...
	if use pam; then
		...
		cp "$(get_erl_libs)/${PAM_P}/priv/bin/epam" \
			"${D}/$(get_erl_libs)/${PF}/priv/bin/epam"
		fowners root:jabber "$(get_erl_libs)/${PF}/priv/bin/epam"
		fperms 4750 "$(get_erl_libs)/${PF}/priv/bin/epam"
	fi
